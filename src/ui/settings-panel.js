...
const TAB_CONTENT = {
  ...
  flight: `
    <h3>Control mode</h3>
    <div class="preset-row">
      <button class="btn subtle" data-mode="acro">Acro (rate)</button>
      <button class="btn subtle" data-mode="angle">Angle (self-level)</button>
    </div>
    <p class="key-list" style="margin-top:0">Acro: stick sets rotation rate, no self-level. Angle: stick sets a target lean angle &amp; climb rate, closed-loop held — like a GPS/camera drone's Normal/Sport mode.</p>
    <h3>Rate profile</h3>
    <div class="preset-row">
    ...
    <h3>Angle mode</h3>
    ${slider('Max tilt angle', 10, 60, 1, 'flight', 'maxTiltAngle')}
    ${slider('Max climb rate', 1, 15, 0.5, 'flight', 'maxClimbRate')}
    <h3>Physics</h3>
    ${slider('Thrust : weight', 1.1, 12, 0.1, 'flight', 'twr')}
    ${slider('Air drag (horizontal)', 0, 2, 0.005, 'flight', 'drag')}
    ${slider('Air drag (vertical)', 0, 2, 0.005, 'flight', 'vertDrag')}
...
export function createSettingsUI(root, tabs, settingsStore, callbacks = {}) {
  ...
  range('flight', 'maxTiltAngle', (v) => v.toFixed(0) + '°');
  range('flight', 'maxClimbRate', (v) => v.toFixed(1) + ' m/s');
  range('flight', 'twr');
  range('flight', 'drag', (v) => v.toFixed(3));
  range('flight', 'vertDrag', (v) => v.toFixed(3));
  range('flight', 'mass');
  ...
  check('camera', 'fisheye', () => callbacks.onCameraChange?.());
  range('camera', 'chaseDist', (v) => v.toFixed(1) + ' m');
  range('camera', 'chaseHeight');

  range('controls', 'audioVolume', (v) => Math.round(v * 100) + '%', () => callbacks.onVolumeChange?.(settingsStore.controls.audioVolume));
  ...
  const syncModeButtons = () => {
    modeButtons.forEach((btn) => {
      btn.classList.toggle('active', btn.dataset.mode === settingsStore.flight.controlMode);
    });
  };
  modeButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
      settingsStore.flight.controlMode = btn.dataset.mode;
      syncModeButtons();
      saveNow();
    });
  });
  syncModeButtons();
  resyncs.push(syncModeButtons);
